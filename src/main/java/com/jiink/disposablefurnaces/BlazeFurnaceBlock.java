...
public class BlazeFurnaceBlock extends DisposableFurnaceBlock {
    public BlazeFurnaceBlock(Settings settings) {
        super(settings);
    }
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new BlazeFurnaceBlockEntity(pos, state);
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, DisposableFurnaces.BLAZE_FURNACE_BLOCK_ENTITY, (world1, pos, state1, be) -> ((DisposableFurnaceBlockEntity) be).tick(world1, pos, state1));
    }
}
