...
public class CharcoalFurnaceBlock extends DisposableFurnaceBlock {
    public CharcoalFurnaceBlock(Settings settings) {
        super(settings);
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, DisposableFurnaces.CHARCOAL_FURNACE_BLOCK_ENTITY, (world1, pos, state1, be) -> ((DisposableFurnaceBlockEntity) be).tick(world1, pos, state1));
    }
}
